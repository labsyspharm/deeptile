...
if __name__ == '__main__':
    ...
    # ground truth: treatment operator and states
    treatment_operator = np.array([[2., 0.], [0., 1.]])
    ...
    print('state 0: {}, estimated {}, cosine distance {:.3E}'.format(
        state_0, result_dict['state_0_pred'].mean(axis=0),
        spatial.distance.cosine(state_0, result_dict['state_0_pred'].mean(axis=0))))
    print('state 1: {}, estimated {}, cosine distance {:.3E}'.format(
        state_1, result_dict['state_1_pred'].mean(axis=0),
        spatial.distance.cosine(state_1, result_dict['state_1_pred'].mean(axis=0))))
    print('treatment operator, cosine distance: {:.3E}, minimization loss {:.3E}'.format(
        spatial.distance.cosine(treatment_operator.flatten(),
            result_dict['minimization_result'].x.flatten()),
        result_dict['minimization_result'].fun))
    print(result_dict['gmm'].converged_, result_dict['gmm'].lower_bound_)
    print('gmm mean, cosine distance: {:.3E}'.format(
        spatial.distance.cosine(true_mean.flatten(),
            result_dict['gmm'].means_[result_dict['sortkey'], ...].flatten())))
    print('gmm cov, cosine distance: {:.3E}'.format(
        spatial.distance.cosine(true_cov.flatten(),
